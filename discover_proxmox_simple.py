...
def main():
    ...
    while True:  # BUCLE INFINITO [PRODUCCION]
        ...
        for i in range(start_index, total_rows):
            ...
            # 1. Si está activo, NO mandar mensaje.
            # 2. Si es la máquina RPA (192.168.49.76), NO mandar mensaje.
            # 3. Cualquier otro error o inicio, SÍ mandar mensaje.
            notify = True
            if resultado.startswith("Activo ("): 
                notify = False
            if "192.168.49.76" in ip:
                notify = False
            ...
            # =====================================================================
            # ESCANEO MANUAL: VMs 152-161
            # =====================================================================
            if vmid == "151":
                print("\n    [INFO] Escaneando VMs 152-161 manualmente...")
                # send_teams_message(browser, page, "[INFO] Escaneando VMs 152-161 manualmente...")
                
                # IPs de las VMs 152-161
                vms_manuales = {
                ...
                for vm_name, ip_vm in vms_manuales.items():
                    ...
                    # FILTRO MANUAL: Activos NO, 49.76 NO. Resto SI.
                    notify_m = True
